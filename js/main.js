...
function init() {
	...
	let config = { alpha: true, premultipliedAlpha: true };
...
function load() {
	...
	if (assetManager.isLoadingComplete() && typeof introAnimation !== 'undefined') {
		...
		if (introAnimation) spineData.state.addAnimation(0, 'Start_Idle_01', false);
...
function render() {
	...
	//gl.pixelStorei(gl.UBPACK_PREMULTIPLY_ALPHA_WEBGL, true);
